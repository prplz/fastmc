...
for pkt in protocol(108).get_packets(HANDSHAKE, SERVERBOUND).values():
    protocol(210).add_packet(HANDSHAKE, SERVERBOUND, pkt)

for pkt in protocol(108).get_packets(STATUS, CLIENTBOUND).values():
    protocol(210).add_packet(STATUS, CLIENTBOUND, pkt)

for pkt in protocol(108).get_packets(STATUS, SERVERBOUND).values():
    protocol(210).add_packet(STATUS, SERVERBOUND, pkt)

for pkt in protocol(108).get_packets(LOGIN, CLIENTBOUND).values():
    protocol(210).add_packet(LOGIN, CLIENTBOUND, pkt)

for pkt in protocol(108).get_packets(LOGIN, SERVERBOUND).values():
    protocol(210).add_packet(LOGIN, SERVERBOUND, pkt)

for i in xrange(0x46):
    protocol(210).add_packet(PLAY, CLIENTBOUND, protocol(108).get_packets(PLAY, CLIENTBOUND)[i])

for i in xrange(0x47, 0x4d):
    pkt = protocol(108).get_packets(PLAY, CLIENTBOUND)[i]
    protocol(210).state(PLAY).from_server(i - 1, pkt.__name__, pkt.src)

for pkt in protocol(108).get_packets(PLAY, SERVERBOUND).values():
